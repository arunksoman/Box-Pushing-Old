...
static void update_speed(int IR_number)
{
	...
	if (IR_number==0)
		left_wheel_speed = left_wheel_speed + MAX_SPEED/1000 * 700;
	else if(IR_number == 7)
		right_wheel_speed = right_wheel_speed + MAX_SPEED/1000 * 700;
	else if(IR_number == 1)
		left_wheel_speed = left_wheel_speed + MAX_SPEED/1000 * 350;
	else if (IR_number == 6)
		right_wheel_speed = right_wheel_speed + MAX_SPEED/1000 * 350;
	else if (IR_number == 2)
	{
		left_wheel_speed = left_wheel_speed + MAX_SPEED/1000 * 550;
		right_wheel_speed = right_wheel_speed - MAX_SPEED/1000 * 300;
	}
    else if (IR_number == 5)
    {
        right_wheel_speed = right_wheel_speed + MAX_SPEED/1000 * 550;
        left_wheel_speed = left_wheel_speed - MAX_SPEED/1000 * 300;
    }
    else if (IR_number == 3)
        left_wheel_speed = left_wheel_speed + MAX_SPEED/1000 * 500;

    else if(IR_number == 4)
        right_wheel_speed = right_wheel_speed + MAX_SPEED/1000 * 500;
}
...
static void push_box(double IR_sensor_value[8], int IR_threshold)
{
	...
	{
		left_wheel_speed = MAX_SPEED * 1;
		right_wheel_speed = MAX_SPEED * 1;
	}
}
